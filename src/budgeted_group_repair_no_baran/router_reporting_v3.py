...
def _build_foundation_k14_report(
    ...
    *,
    ...
    scenario_matrix: Mapping[str, Sequence[float]] | None = None,
) -> dict[str, Any]:
    """Build a frozen foundation-backbone dataset-first report."""

    metrics = root / "metrics"
    matrix_run = scenario_matrix is not None
    variants = tuple(scenario_matrix) if scenario_matrix is not None else ("1", "4")
    scenario_pairs = tuple(
        (variant, float(budget))
        for variant in variants
        for budget in (
            scenario_matrix[variant] if scenario_matrix is not None else (0.2,)
        )
    )
    method_slices = 2 + len(scenario_pairs)
    expected_records = 22_198 * method_slices
    expected_detailed_rows = 9 * method_slices
    expected_metric_rows = 11 * method_slices
    expected_selection_rows = 9 * len(scenario_pairs)
    input_paths = (
        metrics / "per_dataset_f1_matrix.csv",
        metrics / "per_dataset_method_comparison.csv",
        ...
        / "llm"
        / (
            f"router_v3_{backend}_matrix_dry_plan.json"
            if matrix_run
            else f"router_v3_{backend}_dry_plan.json"
        ),
        root / "run_manifest.json",
    )
    f1_rows = _read_csv(input_paths[0])
    ...
    if (
        len(f1_rows) != 9
        or len(detailed_rows) != expected_detailed_rows
        or len(comparison_rows) != expected_comparison_rows
        or len(metric_rows) != expected_metric_rows
        or len(paired_rows) != expected_paired_rows
        or len(selection_rows) != expected_selection_rows
        or int(record_audit.get("records", -1)) != expected_records
        or formal_audit.get("ok") is not True
    ):
        raise ValueError(
    ...
    for row in f1_rows:
        ...
        for variant, budget in scenario_pairs:
            field = (
                f"{backend}_{variant}_{_budget_label(budget)}"
                if matrix_run
                else f"{backend}_{variant}"
            )
            source = (
                f"bgr_{backend}_k{variant}_{_budget_label(budget)}"
                if matrix_run
                else f"bgr_{backend}_k{variant}"
            )
            item[f"{field}_f1"] = _number(row[f"{source}_f1"])
            item[f"{field}_cells"] = _integer(row[f"{source}_llm_cells"])
        matrix.append(item)

    detailed = _prepare_detailed(detailed_rows)
    if matrix_run:
        for prepared, source in zip(detailed, detailed_rows, strict=True):
            if str(source.get("method", "")).startswith("budgeted_group_"):
                prepared["method"] = _sweep_method_label(source)
    aggregate = _prepare_aggregate(metric_rows)
    if matrix_run:
        for prepared, source in zip(aggregate, [
            row for row in metric_rows if str(row.get("scope", "")) in {"micro", "macro"}
        ], strict=True):
            if str(source.get("method", "")).startswith("budgeted_group_"):
                prepared["method"] = _sweep_method_label(source)
    paired = _paired_summary(paired_rows)
    if matrix_run:
        for prepared, source in zip(paired, paired_rows, strict=True):
            prepared["method"] = (
                f"{prepared['method']} · "
                f"{int(round(float(source['budget_share']) * 100))}%"
            )
    costs = _cost_summary(cost_rows)
    wtl_source = (
        [
            row
            for row in detailed_rows
            if not str(row.get("method", "")).startswith("budgeted_group_")
            or abs(float(row.get("budget_share") or 0.0) - 0.2) <= 1e-12
        ]
        if matrix_run
        else detailed_rows
    )
    wtl = _win_tie_loss(wtl_source, backends=(backend,), variants=variants)
    comparison = [
        {
            "dataset": _dataset_name(row),
            "k": str(row["group_size_variant"]),
    ...
    matrix_columns = [
        ...
        *[
            ...
            for column in (
                (
                    f"{backend}_{variant}_{_budget_label(budget)}_f1"
                    if matrix_run
                    else f"{backend}_{variant}_f1",
                    f"{display_name} k={variant} {int(round(budget * 100))}% F1",
                ),
                (
                    f"{backend}_{variant}_{_budget_label(budget)}_cells"
                    if matrix_run
                    else f"{backend}_{variant}_cells",
                    f"{display_name} k={variant} {int(round(budget * 100))}% LLM cells",
                ),
            )
        ],
    ]
    ...
    title = (
        f"Router-v3: {display_name} k=1/2/4/8@20% + k=2/4 budget sweep"
        if matrix_run
        else f"Router-v3: {display_name} 20% budget, k=1/4"
    )
    generated_at = str(
        manifest.get("completed_at")
        or manifest.get("updated_at")
    ...
    markdown = "\n\n".join(
        (
            f"# {title}",
            f"Run: `{root.name}`. 9 datasets, 22,198 error cells, {expected_records:,} final cell records.",
...
<title>{html.escape(title)}</title><style>body{{font:14px/1.5 sans-serif;max-width:1600px;margin:24px auto;padding:24px;color:#172033}}.table-wrap{{overflow:auto}}table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #dce1ea;padding:6px 8px;white-space:nowrap}}th{{background:#eef2f8}}</style></head><body>{html_sections}</body></html>"""
    ...
    artifact = {
        ...
        "dimensions": {
            ...
            "cell_records": expected_records,
            "selection_slices": expected_selection_rows,
            "budget_shares": sorted({budget for _, budget in scenario_pairs}),
            "scenario_matrix": {
                variant: [budget for current, budget in scenario_pairs if current == variant]
                for variant in variants
            },
            "variants": list(variants),
            "backends": [backend],
...
def _build_foundation_matrix_report(
    root: Path,
    validation: Mapping[str, object],
    output_path: str | Path | None,
    *,
    ...
    tabpfn = backend == "tabpfn3"
    return _build_foundation_k14_report(
        root,
        validation,
        output_path,
        backend=backend,
        display_name="TabPFN-3" if tabpfn else "TabICLv2",
        comparison_names=(
            "LightGBM/XGBoost/TabICLv2" if tabpfn else "LightGBM/XGBoost"
        ),
        expected_comparison_rows=108 if tabpfn else 72,
        expected_paired_rows=324 if tabpfn else 288,
...
def build_router_v3_report(
    ...
    if validation.get("router_revision") == ROUTER_V3_TABICLV2_MATRIX_REVISION:
        return _build_foundation_matrix_report(
            root, validation, output_path, backend="tabiclv2"
        )
    if validation.get("router_revision") == ROUTER_V3_TABPFN3_MATRIX_REVISION:
        return _build_foundation_matrix_report(
            root, validation, output_path, backend="tabpfn3"
        )
