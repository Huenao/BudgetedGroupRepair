...
> 实现状态（2026-08）：后续正式实验已收敛为两个独立 revision：`router_v3_tabiclv2_k14_exact_size_conditioned` 与 `router_v3_tabpfn3_k14_exact_size_conditioned`，均固定 20% 预算和 `k=1/4`。TabICLv2 对齐基础 LightGBM/XGBoost run，TabPFN-3 再对齐 TabICLv2 及其冻结树模型 comparison 链。本文后续出现的“双 backend、五 variant、单 revision”内容保留为早期设计背景；当前代码、validator 与 `configs/experiment_router_v3_*_k14.json` 是正式协议来源。
